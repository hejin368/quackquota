...
<#
.SYNOPSIS
    Build and run the QuackQuota Tauri desktop shell for Windows.
...
if (-not $SkipBuild) {
    ...
    try {
        if ($Release) {
            Write-Host "Building QuackQuota (release, no bundle)..." -ForegroundColor Cyan
            & $pnpmCommand.Source run tauri:build
        } else {
            Write-Host "Building QuackQuota (debug, no bundle)..." -ForegroundColor Cyan
...
Write-Host ""
Write-Host "Starting QuackQuota..." -ForegroundColor Green
& $binary
