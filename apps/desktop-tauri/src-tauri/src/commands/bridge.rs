...
pub(crate) fn friendly_provider_error(id: ProviderId, error: &str) -> String {
    ...
    if lower.contains("claude oauth credentials not found") {
        return "Claude sign-in was not found. Run `claude` once to authenticate, then refresh Claude in QuackQuota.".to_string();
    }

    if lower.contains("oauth token expired") || lower.contains("token invalid or expired") {
        return "Claude sign-in expired. Run `claude` to refresh your Claude Code login, then refresh Claude in QuackQuota.".to_string();
    }

    if trimmed == "Authentication required" {
        return "Claude needs sign-in before QuackQuota can read usage. Run `claude` once, or add Claude cookies in Provider settings.".to_string();
    }
...
pub struct SettingsSnapshot {
    ...
    codex_overlay_startup_mode: &'static str,
...
impl From<Settings> for SettingsSnapshot {
    fn from(settings: Settings) -> Self {
        ...
        Self {
            ...
            codex_proxy_use_environment: settings.codex_proxy_use_environment,
            codex_manual_proxy: settings.codex_manual_proxy,
            codex_overlay_startup_mode: settings.codex_overlay_startup_mode.label(),
            monitor_chatgpt_desktop: settings.monitor_chatgpt_desktop,
            show_overlay_on_chatgpt_start: settings.show_overlay_on_chatgpt_start,
            hide_overlay_on_chatgpt_exit: settings.hide_overlay_on_chatgpt_exit,
